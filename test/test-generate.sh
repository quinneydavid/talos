...
MATCHBOX_GROUPS="matchbox/groups"
...
echo -e "\nSample of generated controlplane config (first 20 lines):"
head -n 20 "$MATCHBOX_ASSETS"/controlplane-cp1.yaml 2>/dev/null || echo "No controlplane config generated"

echo -e "\nSample of generated worker config (first 20 lines):"
head -n 20 "$MATCHBOX_ASSETS"/worker-worker1.yaml 2>/dev/null || echo "No worker config generated"
